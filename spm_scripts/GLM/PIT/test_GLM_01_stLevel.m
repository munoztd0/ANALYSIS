...
%% function section
    ...
    function [] = doContrasts(subjoutdir, param, SPM)
        ...
        %weightPos  = ismember(conditionName, {'task-PIT.CSmiusxeffort^1'}) * 1;
        %weightNeg  = ismember(conditionName, {'task-PIT.CSmiusxeffort^1'})* -1;
        %Ct(5,:)    = weightPos+weightNeg;
        
        % con6 
        %Ctnames{6} = 'CSpEffort_CSmEffort&BaselineEffort';
        %weightPos  = ismember(conditionName, {'task-PIT.CSmiusxeffort^1'}) * 1;
        %weightNeg  = ismember(conditionName, {'task-PIT.CSmiusxeffort^1', 'task-PIT.Baselinexeffort^1'})* -1;
