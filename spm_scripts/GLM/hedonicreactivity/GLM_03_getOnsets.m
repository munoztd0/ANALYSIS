...
%for j = 1:length(task)
    ...
    for  i=1:length(subj)
        ...
        %for intensity
        modulators.odor.reward_int  = BEHAVIOR.intensity (strcmp ('chocolate', CONDITIONS));
        modulators.odor.neutral_int = BEHAVIOR.intensity (strcmp ('neutral', CONDITIONS));
        modulators.odor.control_int = BEHAVIOR.intensity (strcmp ('empty', CONDITIONS));
        ...
        for ii = 1:length(name)
            ...
            if strcmp (nameX, 'odor')  % for structure that contains substuctures
                substr = {'reward'; 'control'; 'neutral'; 'reward_int'; 'control_int'; 'neutral_int'};% specify the substructures names ?neutral?
                ...
                for iii = 1:length(substr)
                    ...
                    database.(nameXX) = [num2cell(onsets.(nameX).(substrX)), num2cell(durations.(nameX).(substrX)), num2cell(modulators.(nameX).(substrX)), num2cell(modulators2.(nameX).(substrX))];
                ...
                database.(nameX) = [num2cell(onsets.(nameX)), num2cell(durations.(nameX)), num2cell(modulators.(nameX)), num2cell(modulators2.(nameX))];
                % save the database in a txt file
        ...
        save (mat_name, 'onsets', 'durations', 'modulators', 'modulators2')
