...
do
	# prep for each session's data
		qsub -o /home/REWOD/ClusterOutput -j oe -l walltime=4:00:00,pmem=8GB -M david.munoz@etu.unige.ch -m e -l nodes=1  -q queue1 -N HED_GLM-01_sub-${subj} -F "${subj} ${codeDir} ${matlab_script}" ${matlabSubmit}
