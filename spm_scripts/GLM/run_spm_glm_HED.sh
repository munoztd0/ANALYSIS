...
do
	# prep for each session's data
		qsub -o /home/REWOD/ClusterOutput -j oe -l walltime=1:00:00,pmem=4GB -M david.munoz@etu.unige.ch -m e -l nodes=1  -q queue1 -N HED_GLM-07_s-${subj} -F "${subj} ${codeDir} ${matlab_script}" ${matlabSubmit}
