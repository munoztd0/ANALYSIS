...
do
	...
	qsub -o /home/REWOD/ClusterOutput -j oe -l walltime=1:00:00,pmem=6GB -M eva.pool@unige.ch -m e -l nodes=1 -q queue1 -N prepFEAT_Subject_sub-${subjectID} -F "${subjectID}" ${anatomicalScript}
...
#	for runID in 01
	do
			qsub -o /home/REWOD/ClusterOutput -j oe -l walltime=0:40:00,pmem=6GB -M david.munoz@etu.unige.ch -m e -l nodes=1 -q queue1 -N prepFEAT_Subject_sub-${subjectID}_task-${taskID} -F "${subjectID} ${taskID}" ${functionalScript}
